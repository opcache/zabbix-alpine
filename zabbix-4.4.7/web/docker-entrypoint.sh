...
check_variables_mysql() {
    ...
    if [ "${MYSQL_ALLOW_EMPTY_PASSWORD}" == "true" ] || [ -n "${MYSQL_ROOT_PASSWORD}" ]; then
        ...
        DB_SERVER_ROOT_PASS=${MYSQL_ROOT_PASSWORD:-"zabbix"}
    fi
...
if [ "$1" != "" ]; then
    ...
    echo "** Starting Zabbix frontend1"
...
elif [ "$zbx_type" == "frontend" ] && [ "$zbx_opt_type" == "nginx" ]; then
    echo "** Starting Zabbix frontend2"
